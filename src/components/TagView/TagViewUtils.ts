...
import { ITagView } from 'store/auth/state';
import { router } from 'router/index';
import { store } from 'store/index';
// 构造 tag-view 的元信息，如果符合条件( 不是公共路由 )就提交到 store，生成 tagView 元素
export function addTagView(to, store) {
  // 构造临时 tagView 对象
  const t: ITagView = {
  ...
  if (
    ...
    store.dispatch('auth/addTagView', t);
  }
}

export function setTagView(tagView, store) {
  store.commit('auth/SET_TAG_VIEW', tagView);
}
...
export function removeATagView(state, payload) {
  ...
  if (state.tagView.length === 0) {
    ...
    if (payload === state.tagView.length && route.path === removedTagView) {
      router.push(state.tagView[payload - 1].fullPath);
      return;
    }
    // 如果移除的是第一个 tagView 则路由跳转移除后的第一个 tagView
    if (payload === 0 && route.path === removedTagView) {
      router.push(state.tagView[0].fullPath);
      return;
    }
    if (route.path === removedTagView) {
      router.push(state.tagView[payload - 1].fullPath);
    }
  }
}
